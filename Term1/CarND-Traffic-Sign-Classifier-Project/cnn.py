...
def LeNet(x, n_classes, use_dropout=False, keep_prob=0.7):
    ...
    with tf.variable_scope('conv2'):
        ...
        print('Conv 2 shape {}'.format(conv2.shape))
    ...
    with tf.variable_scope('fc1'):
        ...
        fc1_W = tf.Variable(tf.truncated_normal(shape=(256, 200), mean=mu, stddev=sigma), name='weights')
        fc1_b = tf.Variable(tf.zeros(200), name='biases')
    ...
    with tf.variable_scope('fc2'):
        ...
        fc2_W = tf.Variable(tf.truncated_normal(shape=(200, 100), mean=mu, stddev=sigma), name='weights')
        fc2_b = tf.Variable(tf.zeros(100), name='biases')
    ...
    with tf.variable_scope('fc3'):
        ...
        fc3_W = tf.Variable(tf.truncated_normal(shape=(100, n_classes), mean=mu, stddev=sigma), name='weights')
